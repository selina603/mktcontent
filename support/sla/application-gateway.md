...
我们保证每个具有两个或更多中型或更大型实例的应用程序网关服务至少在 99.9% 的时间内都可用。我们不为只有一个实例或小型实例的应用程序网关服务提供服务级别协议。
...
“**应用程序网关服务**”是指由配置为执行 HTTP 负载平衡服务的一个或多个应用程序网关实例组成的集合。

###应用程序网关月度正常运行时间计算和服务级别
1. “**最大可用分钟数**”是指某个计费月份中在 Azure 订阅中部署了由两个或更多中型或更大的应用程序网关实例组成的应用程序网关服务的累计总分钟数。

2. “**停机时间**”是指给定应用程序网关服务在计费月份中不可用的累计最大可用总分钟数。如果在给定分钟期间内与应用程序网关服务的所有连接尝试均失败，则该分钟视为不可用。

3. 给定应用程序网关服务的“**月度正常运行时间百分比**”计算方法为：应用程序网关服务在计费月份的最大可用分钟数减去停机时间，然后除以最大可用分钟数。月度正常运行时间百分比的计算公式如下：

		月度正常运行时间百分比 =   (最大可用分钟数 - 停机时间) / 最大可用分钟数 

4. 客户在使用每个应用程序网关服务时适用以下服务级别和服务信用。
